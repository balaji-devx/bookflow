    ...
    >
      <div
        className={`action-top ${
          isPrimary ? "action-top-primary" : "action-top-secondary"
        }`}
      >
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      try {
        // 1. Fetch all three activity types concurrently
        const [ordersRes, borrowsRes, submissionsRes] = await Promise.all([
          ...
          fetch(`${API_BASE_URL}/lend/user/submissions`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        ...
        // Map Lend Submissions
        const mappedSubmissions = submissionsData.map((submission) => ({
          type: `Lending Submission`,
          desc: submission.title || `ISBN: ${submission.isbn}`,
  ...
  // --- DEFINING ACTION CARDS ---
  const actions = [
    {
      title: "Continue Shopping",
      icon: Icon.Cart,
      description: "View available books to purchase and add to cart.",
    ...
    },
    {
      title: "Lend Books",
      icon: Icon.Lend,
      description: "Submit your books to the library for review.",
      onClick: () => navigate("/lend"),
      isPrimary: false,
    },
  ];

  // Check if user is admin to add the Admin Dashboard card
  if (localStorage.getItem("userRole") === "admin") {
    actions.unshift({
      title: "Admin Dashboard",
      icon: Icon.Settings,
      description: "Manage inventory, orders, borrows, and users.",
      ...
      <main className="profile-page">
        <header className="profile-header">
          <div className="header-left">
            <h1 className="profile-title">
              Hello, {name.split(" ")[0] || "Guest"} 
            </h1>
            <p className="profile-sub">Welcome!</p>
          </div>
        </header>
        ...
        <section className="profile-card" aria-labelledby="account-info">
          ...
          <div className="profile-card-right">
            <h2 id="account-info" className="user-name">
              {name}
            </h2>
            ...
            {isLoadingActivity ? (
              ...
              recentActivity.map((act, i) => (
                <div className="activity-item" key={i}>
                  ...
                  <div
                    className={`activity-status ${act.statusClass}`}
                    aria-live="polite"
                  >
                    {act.status}
                  </div>
